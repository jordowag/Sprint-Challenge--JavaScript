...
let universities = graduates.map(person => person.university);
console.log(universities)
// using const: use a for each 
// const universities = [];
...
const contactInfo = [];
graduates.forEach(person => (contactInfo.push(`${person.first_name} ${person.email}`))); 
console.log(contactInfo);


/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called uni that contains them all. Log the result. */
let uni = [];
uni = graduates.filter(person => person.university.includes("Uni"));
uni = uni.map(person => person.university);
console.log(uni);
